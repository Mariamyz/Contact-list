...
const intialState = {
  ...
  filterStatus: "",
  contactStatuss: {
    work: { count: 0, bg: "#c8e6c9" },
    family: { count: 0, bg: "#ffecb3" },
    private: { count: 0, bg: "#c5cae9" },
    friends: { count: 0, bg: "#b3e5fc" },
    others: { count: 0, bg: "#e0e0e0" },
  },
};
...
const reducer = (state = intialState, action) => {
  switch (action.type) {
    ...
    case DELETE_STATUS:
      ...
      const deleteContactStatus =
        newContactStatussAfterDelete[action.payload].count;
    ...
    case EDIT_STATUS:
      ...
      const updatedContacts = state.contacts.map((contact) =>
        contact.status === oldStatusName
          ? { ...contact, status: editStatusName }
          : contact,
      );
      return {
        ...state,
